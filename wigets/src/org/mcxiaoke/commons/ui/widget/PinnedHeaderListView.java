...
import org.mcxiaoke.commons.ui.ViewCompatUtils;
...
public class PinnedHeaderListView extends AutoScrollListView implements
	...
	private void ensurePinnedHeaderLayout(int viewIndex) {
		...
		if (view.isLayoutRequested()) {
			int widthSpec = MeasureSpec.makeMeasureSpec(mHeaderWidth,
					MeasureSpec.EXACTLY);
			int heightSpec;
	...
	private boolean smoothScrollToPartition(int partition) {
		...
		ViewCompatUtils.smoothScrollToPositionCompat(this, position
				+ getHeaderViewsCount(), offset);
		// smoothScrollToPositionFromTop(position + getHeaderViewsCount(),
		// offset);
		return true;
	}
