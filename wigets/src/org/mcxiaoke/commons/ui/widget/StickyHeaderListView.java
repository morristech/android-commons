...
package org.mcxiaoke.commons.ui.widget;
 ...
 * Notes: 1. The class uses the first available child ListView as the working
 ...
 * is specified, the class will try to extract it from the ListView 3. The class
 * registers itself as a listener to scroll events (OnScrollListener), if the
 * ListView needs to receive scroll events, it must register its listener using
 * this class' setOnScrollListener method. 4. Headers for the list view must be
 * added before using the StickyHeaderListView 5. The implementation should
 * register to listen to dataset changes. Right now this is not done since a
 * change the dataset in a listview forces a call to OnScroll. The needed code
 * is commented out.
 */
// from D:\android\source\packages\apps\Calendar\src\com\android\calendar
public class StickyHeaderListView extends FrameLayout implements
		OnScrollListener {
	...
	protected View mDummyHeader = null; // A invisible header used when a
	...
	protected int mCurrentSectionPos = -1; // Position of section that has its
											// header on the
											// top of the view
	protected int mNextSectionPosition = -1; // Position of next section's
												// header
	protected int mListViewHeadersCount = 0;

	/**
	 * Interface that must be implemented by the ListView adapter to provide
	 * headers locations and number of items under each header.
	 * 
	 */
	public interface HeaderIndexer {
		/**
		 * Calculates the position of the header of a specific item in the
		 * adapter's data set. For example: Assuming you have a list with albums
		 * and songs names: Album A, song 1, song 2, ...., song 10, Album B,
		 * song 1, ..., song 7. A call to this method with the position of song
		 * 5 in Album B, should return the position of Album B.
		 * 
		 * @param position
		 *            - Position of the item in the ListView dataset
		...
		/**
		 * Calculates the number of items in the section defined by the header
		 * (not including the header). For example: A list with albums and
		 * songs, the method should return the number of songs names (without
		 * the album name).
		 * 
	 ...
	 *            - the index (in the list's adapter) of the top visible item.
	...
	/**
	 * Sets a separator below the sticky header, which will be visible while the
	 * sticky header is not scrolling up.
	 * 
	 * @param color
	 *            - color of separator
	 * @param width
	...
	public void setHeaderSeparator(int color, int width) {
		...
		ViewGroup.LayoutParams params = new LayoutParams(
				LayoutParams.MATCH_PARENT, width, Gravity.TOP);
	...
	protected void updateStickyHeader(int firstVisibleItem) {
		...
		if (mAdapter != null && mIndexer != null && mDoHeaderReset) {
			...
			int sectionSize = 0;
			int sectionPos = mIndexer
					.getHeaderPositionFromItemPosition(firstVisibleItem);
			...
			if (sectionPos != mCurrentSectionPos) {
				...
				if (sectionPos == -1) {
					...
					View v = mAdapter.getView(sectionPos
							+ mListViewHeadersCount, null, mListView);
					v.measure(MeasureSpec.makeMeasureSpec(mListView.getWidth(),
							MeasureSpec.EXACTLY), MeasureSpec.makeMeasureSpec(
							mListView.getHeight(), MeasureSpec.AT_MOST));
					this.removeView(mStickyHeader);
					mStickyHeader = v;
					newView = true;
				}
			...
			// Do transitions
			// If position of bottom of last item in a section is smaller than
			// the height of the
			// sticky header - shift drawable of header.
			if (mStickyHeader != null) {
				int sectionLastItemPosition = mNextSectionPosition
						- firstVisibleItem - 1;
				...
				if (mHeaderHeightListener != null
					...
					mLastStickyHeaderHeight = stickyHeaderHeight;
					mHeaderHeightListener
							.OnHeaderHeightChanged(stickyHeaderHeight);
				}

				View SectionLastView = mListView
						.getChildAt(sectionLastItemPosition);
				if (SectionLastView != null
						&& SectionLastView.getBottom() <= stickyHeaderHeight) {
					...
					mStickyHeader.setTranslationY(lastViewBottom
							- stickyHeaderHeight);
					if (mSeparatorView != null) {
						mSeparatorView.setVisibility(View.GONE);
					}
					...
					if (mSeparatorView != null
							&& !mStickyHeader.equals(mDummyHeader)) {
				...
				if (newView) {
					...
					if (mSeparatorView != null
						...
						FrameLayout.LayoutParams params = new FrameLayout.LayoutParams(
								LayoutParams.MATCH_PARENT, mSeparatorWidth);
						params.setMargins(0, mStickyHeader.getMeasuredHeight(),
								0, 0);
						mSeparatorView.setLayoutParams(params);
						mSeparatorView.setVisibility(View.VISIBLE);
					}
	...
	private void setChildViews() {
		...
		ViewGroup.LayoutParams params = new LayoutParams(
				LayoutParams.MATCH_PARENT, 1, Gravity.TOP);
