...
/**
 * <p>
 * A button with more than two states. When the button is pressed or clicked,
 ...
 * </p>
 * <p>
 * See {@link R.styleable#MultiStateButton MultiStateButton Attributes},
...
public class MultiStateButton extends Button {
	...
	// The currently displaying resource ID. This gets set to a default on
	// creation and remains
	// on the last set if the resources get set to null.
	private int mButtonResource;
	// A list of all drawable resources used by this button in the order it uses
	// them.
	 ...
	 * 
	 * This sets the maximum states allowed to the length of the resources
	 * array. It will also set the current state to the maximum allowed if it's
	 * greater than the new max.
	 */
	public void setButtonResources(int[] resources)
			throws IllegalArgumentException {
		if (resources == null) {
			throw new IllegalArgumentException(
					"Button resources cannot be null");
		}
	...
	public boolean setState(int state) {
		if (state >= mMaxStates || state < 0) {
			...
			Log.w("Cal",
					"MultiStateButton state set to value greater than maxState or < 0");
			return false;
		}
		mState = state;
		setButtonDrawable(mButtonResources[mState]);
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mButtonDrawable != null) {
			...
			final int horizontalGravity = getGravity()
