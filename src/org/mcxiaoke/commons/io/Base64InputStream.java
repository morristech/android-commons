...
package org.mcxiaoke.commons.io;
...
// from source\packages\apps\Exchange
public class Base64InputStream extends InputStream {
	...
	/**
	 * Retrieve data from the underlying stream, decode it, and put the results
	 * in the byteq.
	 * 
	 * @throws IOException
	 */
	private void fillBuffer() throws IOException {
		...
		while (!done) {
			...
			// START EAS CHANGES
			case 0:
				// In EAS, a zero will indicate the end of the (inline) base64
				// string
				// Stop reading at this point, so that we can continue with
				// WBXML
				done = true;
				return;
				// END EAS CHANGES
			case -1:
				// No more input - just return, let outputBuffer drain out, and
				// be done
				return;
			case '=':
				// START EAS CHANGES
				// Allow for a second '=' before we're really done (we should
				// get a zero next)
				if (padSeen) {
					return;
				}
				// We've seen a (first) end padding character, flush what's in
				// the buffer
				padSeen = true;
				// END EAS CHANGES
	...
	private void decodeAndEnqueue(int len) {
		...
		// There's a bit of duplicated code here because we want to have
